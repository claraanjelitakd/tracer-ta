<!--
  Halaman Detail & Audit Kuesioner Alumni (Biro 3)
  ...
  Struktur: 3 Tab Utama (1. Detail Profile, 2. Kuesioner Univ, 3. Kuesioner Prodi) + Sinkronisasi LinkedIn
-->
<script setup>
...
const hasLinkedIn = computed(() => {
    return !!(props.biodata?.linkedin_username || props.biodata?.linkedin_url || props.formData?.linkedin_username || props.formData?.linkedin_url);
});
...
const startSync = async () => {
    ...
    try {
        const response = await fetch(`/biro3/alumni/${props.biodata.id}/sync-linkedin`, {
            method: 'POST',
            headers: {
        ...
        if (data.success) {
            ...
            syncForm.current_job = data.data.current_job || '';
            syncForm.current_company = data.data.current_company || '';
            syncForm.industry = data.data.industry || '';
            syncForm.location = data.data.location || '';
        } else {
...
const saveSync = () => {
    syncForm.post(`/biro3/alumni/${props.biodata.id}/save-linkedin`, {
        preserveScroll: true,
        onSuccess: () => {
...
<template>
    <Head :title="`Detail Mahasiswa - ${biodata.nama || biodata.nim} - Biro 3`" />
    ...
    <div class="min-h-screen bg-[#f8fafc] text-gray-800 font-sans flex">
        ...
        <div class="flex-1 flex flex-col min-w-0 lg:pl-72">
            ...
            <header class="bg-[#0D542B] text-white pt-8 pb-16 px-4 sm:px-6 lg:px-8">
                <div class="w-full max-w-[1400px] mx-auto flex flex-col md:flex-row md:items-center justify-between gap-6">
                    ...
                    <div class="flex flex-wrap items-center gap-3 shrink-0">
                        <a 
                            :href="`/biro3/alumni/${biodata.id}/export-excel`"
                            target="_blank"
            ...
            <main class="w-full max-w-[1400px] mx-auto -mt-10 px-4 sm:px-6 lg:px-8 space-y-6 pb-16">
                ...
                <!-- ============================================================= -->
                <!-- TAB 1: DETAIL PROFILE & SINKRONISASI LINKEDIN                 -->
                <!-- ============================================================= -->
                <div v-if="activeTab === 'profile'" class="space-y-6">
                    
                    <!-- Panel LinkedIn MCP Agent -->
                    <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-gray-100 pb-4 mb-4">
                            <div>
                                <h3 class="text-sm font-extrabold text-[#0077b5] flex items-center gap-2">
                                    <span>Sinkronisasi Data Karir LinkedIn</span>
                                    <span class="text-[10px] bg-sky-50 text-[#0077b5] px-2 py-0.5 rounded font-mono font-bold">MCP Agent</span>
                                </h3>
                                <p class="text-xs text-gray-500 mt-0.5">Tarik data perusahaan dan jabatan kerja mutakhir langsung dari LinkedIn alumni.</p>
                            </div>
                            ...
                            <button 
                                ...
                                class="px-4 py-2 rounded-xl text-xs font-bold text-white bg-[#0077b5] hover:bg-[#005c8a] disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-sm shrink-0 cursor-pointer"
                            >
                                <span v-if="isSyncing">Sedang Menarik Data...</span>
                                <span v-else>Tarik Data Pekerjaan Terkini</span>
                            </button>
                        </div>

                        <div class="text-xs text-gray-600 flex items-center gap-2">
                            <span class="font-bold">Akun LinkedIn:</span>
                            <span class="font-mono bg-gray-50 px-2 py-1 rounded border border-gray-200">
                                {{ formData.linkedin_username || formData.linkedin_url || 'Belum dicantumkan' }}
                            </span>
                        </div>
                        ...
                        <!-- Tinjauan Data Hasil Sinkronisasi -->
                        <div v-if="syncResult" class="mt-4 p-5 bg-sky-50/70 border border-[#0077b5]/30 rounded-xl space-y-4">
                            <h4 class="text-xs font-extrabold text-[#0077b5] uppercase tracking-wider">Hasil Ekstraksi Data</h4>
                            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 text-xs">
                                <div>
                                    <label class="block font-bold text-gray-700 mb-1">Jabatan / Posisi</label>
                                    <input type="text" v-model="syncForm.current_job" class="w-full text-xs rounded-lg border-gray-300 bg-white" />
                                </div>
                                <div>
                                    <label class="block font-bold text-gray-700 mb-1">Nama Perusahaan</label>
                                    <input type="text" v-model="syncForm.current_company" class="w-full text-xs rounded-lg border-gray-300 bg-white" />
                                </div>
                                <div>
                                    <label class="block font-bold text-gray-700 mb-1">Sektor Industri</label>
                                    <input type="text" v-model="syncForm.industry" class="w-full text-xs rounded-lg border-gray-300 bg-white" />
                                </div>
                                <div>
                                    <label class="block font-bold text-gray-700 mb-1">Lokasi</label>
                                    <input type="text" v-model="syncForm.location" class="w-full text-xs rounded-lg border-gray-300 bg-white" />
                                </div>
                            </div>
                            <div class="flex justify-end gap-2 pt-2">
                                <button type="button" @click="syncResult = null" class="px-3 py-1.5 bg-white border border-gray-300 text-xs rounded-lg font-bold">Batal</button>
                                <button type="button" @click="saveSync" :disabled="syncForm.processing" class="px-4 py-1.5 bg-[#0D542B] text-white text-xs rounded-lg font-bold">Simpan ke Basis Data</button>
                            </div>
                        </div>
                    </div>

                    <!-- Rekap Profil Lengkap (Read-only Table) -->
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                            ...
                            <dl class="divide-y divide-gray-100 text-xs">
                                ...
                                <div class="py-2.5 grid grid-cols-3">
                                    <dt class="text-gray-500 font-semibold">NIK</dt>
                                    <dd class="col-span-2 font-mono text-gray-900">{{ formData.nik || '-' }}</dd>
                                </div>
                            </dl>
                        </div>
