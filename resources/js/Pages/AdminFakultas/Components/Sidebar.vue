<!--
  Komponen Navigasi Sidebar Terpadu Admin Fakultas (Sidebar.vue)
  File: resources/js/Pages/AdminFakultas/Components/Sidebar.vue
...
const currentUser = computed(() => {
    return props.user?.name ? props.user : (page.props.auth?.user || { name: 'Admin Fakultas', email: 'fakultas@ukdw.ac.id' });
});

const currentFakultas = computed(() => {
    return props.fakultas?.nama_fakultas ? props.fakultas : (page.props.auth?.user?.fakultas || {});
});

const userInitials = computed(() => {
    const name = currentUser.value?.name || 'FK';
    return name.split(' ').map(n => n[0]).slice(0, 2).join('').toUpperCase();
});

const isDashboardActive = computed(() => {
    return page.url === '/fakultas' || page.url === '/fakultas/' || page.url.startsWith('/fakultas/dashboard');
});
...
const handleLogout = () => {
    Swal.fire({
        ...
        text: 'Apakah Anda yakin ingin keluar dari sesi Admin Fakultas?',
        icon: 'question',
    ...
    <div>
        ...
        <header class="lg:hidden bg-white/95 backdrop-blur-md border-b border-gray-200/80 sticky top-0 z-40 px-4 py-3 flex items-center justify-between shadow-2xs">
            <div class="flex items-center gap-3">
                ...
                <Link href="/fakultas/dashboard" class="flex items-center gap-2">
                    ...
                    <div>
                        ...
                        <span class="text-[10px] font-bold text-[#0D542B] uppercase tracking-wider">{{ currentFakultas.nama_fakultas || 'Admin Fakultas' }}</span>
                    </div>
                </Link>
            </div>
        ...
        >
            ...
            <div>
                ...
                <div class="p-6 border-b border-gray-100 flex items-center justify-between">
                    <Link href="/fakultas/dashboard" class="flex items-center gap-3.5 group">
                        ...
                        <div class="min-w-0 flex-1">
                            ...
                            <div class="flex items-center gap-1.5 mt-0.5">
                                ...
                                <span class="text-[11px] font-extrabold text-[#0D542B] uppercase tracking-wider block leading-tight break-words">
                                    {{ currentFakultas.nama_fakultas || 'Fakultas' }}
                                </span>
                            </div>
                        </div>
                    </Link>
                ...
                <div class="px-4 py-6 space-y-1">
                    ...
                    >
                        ...
                        <div class="flex-1">
                            <span>Data Alumni Fakultas</span>
                        </div>
                    </Link>
                </div>
            </div>
                    ...
                    <div class="flex items-center gap-3 min-w-0 flex-1">
                        ...
                        <div class="min-w-0 flex-1">
                            ...
                            <span class="block text-[10px] text-gray-500 break-words leading-tight mt-0.5">
                                {{ currentFakultas.nama_fakultas || currentUser.email }}
                            </span>
                        </div>
                    </div>
